...
- `VITE_APP_IDENTITY_BASE_URL`: Identity Server의 베이스 주소
- `VITE_APP_IDENTITY_CLIENTID`: 클라이언트 아이디
- `VITE_APP_IDENTITY_REDIRECT_URI`: 로그인 후 리다이렉트될 주소 (보통 /callback)
- `VITE_APP_IDENTITY_SCOPE`: 접근 권한 범위 (openid profile email 등)
- `VITE_APP_IDENTITY_TYPE`: 응답 타입 (code 등)
- `VITE_APP_IDENTITY_CLIENTSECRET`: 클라이언트 시크릿, 상황에 따라 필요할 수도 있음.
- `VITE_APP_IDENTITY_COOKIE`: 쿠키를 사용할 때, 쿠키 이름
- `VITE_APP_SITE_TITLE`: 프로젝트의 Title 태그에 넣을 내용
- `VITE_APP_SERVICE_URL`: 프로젝트의 기본 URL
- `VITE_API_BASE_URL`: 인증서버의 API 베이스 URL
- `VITE_API_BOARD_API_URL`: 비즈니스로직을 제공하는 API 베이스 URL
...
- **ts** 모든 파일은 TypeScript 기반으로 작성합니다.  .js 파일은 사용하지 않습니다.
- 가급적 any 타입의 사용을 지양합니다.  최대한 interface를 명확히 구현합니다.
- **하나의 파일에 너무 많은 코드를 사용하지 말고, 적절히 파일을 나눕니다. 라우트가 변경되면 항상 새로운 파일로 접근합니다.**
