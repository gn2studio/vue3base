...
export class GraphEngine {
    ...
    constructor(canvasElement: HTMLCanvasElement) {
        ...
        if (!context) {
            throw new Error('Could not get 2d context for GraphEngine');
        }
        this.ctx = context;
    ...
    private update(): void {
        for (let i = 0; i < this.particles.length; i++) {
            ...
            if (distance < this.mouse.radius) {
                ...
                const directionX = forceDirectionX * force * this.config.mouseRepelForce;
                const directionY = forceDirectionY * force * this.config.mouseRepelForce;
